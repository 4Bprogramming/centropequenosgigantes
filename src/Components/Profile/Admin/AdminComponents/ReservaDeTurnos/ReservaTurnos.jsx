...
function ReservaTurnos({ token, rol, profesional, email }) {
  ...
  let filterDay = Array.from(
    turnosTodos.map((turnos) => turnos.date.split("-").reverse().join("-"))
  );
  let turnosDisponibles = turnosTodos.filter(
  ...
  if (profesional) {
    ...
    turnosDisponibles = turnosProfesional?.filter(
      (turno) => turno?.estado === "disponible"
    );
  }
  ...
  const handleSelect = (seletedOptions) => {
    ...
    setPost({
      ...post,
      profesionalIdProfesional: seleccion2,
  ...
  function handleChangeDate(e) {
    ...
    if (rol === "usuario") {
      let TurnosDiaElegido = turnosDisponibles.filter(
        (turnos) => turnos.date === eleccion
      );
      setPost({
        ...post,
      ...
      let TurnosDiaElegido = turnosDisponibles.filter(
        (turnos) =>
          turnos.profesionalIdProfesional === post.profesionalIdProfesional &&
          turnos.date === eleccion
      );
      setPost({
        ...post,
        turnosMostrar: TurnosDiaElegido,
